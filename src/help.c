...
/* Public variable */
osh_command_t cmd_help = { NAME, BRIEF, SYNOPSIS, DESCRIPTION, osh_help };
  ...
  if (! quiet)
    {
      ...
      args_print_rows (names, osh_screen_cols ());
      argsclear (names);
    }
}
...
static void help_list (bool quiet)
{
  ...
  for (i = 0; ! quiet && i < cmd_size (); i ++)
    {
      osh_command_t * cmd = cmd_lookup (i);
      printf ("%-*s\t%s\n", HELPINDENT, cmd -> name, cmd -> brief);
    }
}


static void print_help (osh_command_t * cmd, bool man, bool quiet)
{
  if (! quiet)
    {
      if (! man)
  ...
  if (optind < argc)
    {
      ...
      while (optind < argc)
	{
	  osh_command_t * cmd = cmd_by_name (argv [optind]);
	  if (cmd)
